...
/*Task 1: counterMaker()
  ...
  A: 1 is an overhead function, 2 is a callback function.
  
  2. Which of the two uses a closure? How can you tell?
  A: 1 === function scoped && 2 === globally scoped.
...
function scoreboard(cbInningScore, cbRandomNumber, num) {
  const finalArr = [];
  let finalHome = 0;
  let finalAway = 0;
  ...
  for (let i = 1; i < num + 1; i++) {
    ...
    finalHome += home;
    finalAway += away;
    finalArr.push(`Inning ${i}: Away ${away} - Home ${home}`);
  }

  if (finalHome === finalAway) {
    finalArr.push(
      `This game will require extra innings: Away ${finalHome} - Home ${finalAway}`
    );
  } else {
    finalArr.push(`Final Score: Away ${finalAway} - Home ${finalHome}`);
  }
  return finalArr;
}
